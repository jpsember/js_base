...
require 'test/unit'
require 'js_base'
req 'io_recorder swizzler'

$_original_directory = nil
$_test_dir = nil
$_saved_stdout = nil
...
def enter_test_directory(subdirectory_name = nil)
  ...
  $_test_dir = File.join(script_path,subdirectory_name)
  FileUtils.mkdir_p($_test_dir)

  $_original_directory = Dir.pwd
  Dir.chdir($_test_dir)
end
...
def leave_test_directory(retain = false)
  raise IllegalStateException, "No test directory found" if !$_original_directory
  Dir.chdir($_original_directory)
  $_original_directory = nil
  FileUtils.rm_rf($_test_dir) if !retain
end

# Redirect stdout to a string buffer
#
def redirect_stdout
  raise IllegalStateException, "Already redirected" if $_saved_stdout
